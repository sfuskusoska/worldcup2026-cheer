...
3. **優勝オッズ表**（全31チーム、順位・オッズ・確率・予想pt・持ち主列）。**折り畳み式**（ネイティブ`<details id="oddsDetails">`、デフォルト閉じる）。閉じた状態でも`renderOdds()`が`#oddsSummaryText`に「優勝オッズ表を開く（全31チーム・現在の本命: 〇〇 +xxx）」とODDS[0]（現在の1位）を動的表示するので、開かなくても本命だけは分かる。開閉トグルはブラウザネイティブ機能でJS不要（矢印アイコンの回転のみCSS）。
...
13. **🎬 おすすめ動画**（ページ最下部・戦術ボードの直後、旧称「ハイライト動画」）：YouTube・X（旧Twitter）を同じセクションにまとめて掲載。**両方ともコード内配列にURLを手動登録する方式**（外部API不要）。
    - **YouTube**（`renderVideoGrid`/`playVideo`/`parseYoutubeId`、配列`YOUTUBE_VIDEOS`）：エントリは`{ url: 'https://www.youtube.com/watch?v=...' , title: '...' }`または`{ id: '...', title: '...' }`のどちらでも可（`youtu.be`短縮URL・`/watch?v=`・`/embed/`・`/shorts/`・生の11文字IDいずれも`parseYoutubeId()`で解析、不正な入力は静かに除外されクラッシュしない）。サムネイルは`https://i.ytimg.com/vi/{id}/hqdefault.jpg`（追加フェッチ不要）。**クリックでサムネイルをYouTube埋め込みiframe（autoplay=1）に置き換える**遅延読み込み方式（ページ読み込み時にYouTube本体を叩かない。2回クリックしてもiframeが重複しないようガード済み）。
    - **X（旧Twitter）**（`renderXPosts`/`extractXStatusUrl`/`loadTwitterWidgetsScript`、配列`X_POSTS`）：投稿URL（`x.com`または`twitter.com`の`/status/数字`形式）を並べるだけ。YouTubeと違い**X公式の埋め込みスクリプト**（`https://platform.twitter.com/widgets.js`、初回のみ動的挿入・`twitterWidgetsPromise`でキャッシュ）に依存する方式で、サムネイル画像だけを事前取得する軽量な仕組みはXには存在しないため、`<blockquote class="twitter-tweet" data-theme="dark">`をXの投稿カード（iframe）へその場で変換させる。**実データで検証済み**：Xの公式サンプル投稿（`twitter.com/Twitter/status/20`）でblockquote→`platform.twitter.com/embed/Tweet.html`への実際のiframe変換まで確認。⚠️**プレビュー環境の制約**：iframeの高さがXの非同期postMessageハンドシェイクで確定するため、Claude Previewのヘッドレス環境では高さ0のまま止まることがある（紙吹雪演出と同種の既知の制約）。実機・本番環境での見た目確認を推奨。
    - 統合表示は`renderRecommendedVideos()`が担当：YouTube/Xそれぞれの件数を見て、**両方存在する時だけ**「YouTube」「X（旧Twitter）」のサブラベルを出し分け、両方空なら「まだ動画が登録されていません」を表示。`init()`から呼ぶのはこの関数（`renderVideoGrid()`/`renderXPosts()`を個別に呼ぶと空表示ロジックが崩れるので注意）。

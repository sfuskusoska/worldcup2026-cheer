...
13. **🎬 ハイライト動画**（ページ最下部・戦術ボードの直後、`renderVideoGrid`/`playVideo`/`parseYoutubeId`）：YouTubeのサムネイル一覧。**動画はコード内`YOUTUBE_VIDEOS`配列にURL/動画IDを手動登録する方式**（外部API不要。デフォルトは空配列で「まだ動画が登録されていません」を表示）。エントリは`{ url: 'https://www.youtube.com/watch?v=...' , title: '...' }`または`{ id: '...', title: '...' }`のどちらでも可（`youtu.be`短縮URL・`/watch?v=`・`/embed/`・`/shorts/`・生の11文字IDいずれも`parseYoutubeId()`で解析、不正な入力は静かに除外されクラッシュしない）。サムネイルは`https://i.ytimg.com/vi/{id}/hqdefault.jpg`（追加フェッチ不要）。**クリックでサムネイルをYouTube埋め込みiframe（autoplay=1）に置き換える**遅延読み込み方式（ページ読み込み時にYouTube本体を叩かない。2回クリックしてもiframeが重複しないようガード済み）。検証済み：3種類のURL形式すべて正しくID抽出、クリック→埋め込み切り替え、不正入力での非クラッシュ、モバイル幅グリッド崩れなし。
